...
c = [ #matrix c
  [],
  [0, 2, 4, infinity, infinity, 7, infinity], #t = 1 row
  [2, 0, 3, 3, infinity, infinity, infinity], #u = 2 row
  [4, 3, 0, 4, 3, 8, infinity], #v = 3 row
  [infinity, 3, 4, 0, 6, infinity, infinity], #w = 4 row
  [infinity, infinity, 3, 6, 0, 6, 8], #x = 5 row
  [7, infinity, 8, infinity, 6, 0, 12], #y = 6 row
  [infinity, infinity, infinity, infinity, 8, 12, 0], #z = 7 row
]

nodes = 7

distance = [[0] * (N + 1) for _ in range (N + 1)] #creating a 2d matrix
...
while True:
  ...
  for i in range(1, N + 1): #destination
    for j in range(1, N + 1): #node j's vector
      best = distance[i][j] #updates each D(i, j)

      for k in range (1, N + 1): #neighbors
